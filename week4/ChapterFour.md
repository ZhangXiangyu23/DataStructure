...
==问题来了，为什么要寻找右子树中的最小值呢？==

因为这样将这个值提上去之后，可以满足一定大于左子树，而且小于右子树。符合二叉搜索树的特性。

##### 同样的道理，也可以选择左子树中的最大值，这样提上去之后，既可以大于左子树，而且也小于右子树！！！

2）==只有一棵子树==:

只有左子树：一个节点如果只有左子树，那么它的右子树肯定是指向为NULL，删除它之后还要保持它的子树在啊！所以，先将T保存在临时变量中，等待之后删除。让被删除节点的父节点指向它的右子树，其实就是让存在的的左子树替代被删除的节点T=T->Left，从而保证删除了被删除节点，而且保持不变。

只有右子树，也是同样的理解。将T先保存在临时变量中，然后T=T->Right;

最后将保存在临时变量中的指针，free掉。
...
SearchTree Delete(ElementType X,SearchTree T)
{
	...
	{
		temp=FindMin(T->Right);//找到右子树中最小的值
		T->Element=temp->Element;//覆盖原来的值
		T->Right=Delete(T->Element,T->Right);//递归地删除右子树中的那个值
	}
	else
	{
		temp=T;
		if(T->Left==NULL)//只有右子树
		{
			T=T->Right;
		}
...
AVL树的命名是用两个人名的首字母拼起来命名的。这个了解就好。AVL树指的是自平衡二叉查找树。
...
树的深度是指：从根节点走起，到达指定节点，需要几条路径（也就是几条边）

树的高度是指：该节点子树的叶子节点开始，从下到上，到达该节点，需要几条边。
...
![image-20211215173913196](ChapterFour.assets/image-20211215173913196.png)
...
![image-20211215193119234](ChapterFour.assets/image-20211215193119234.png)
...
![image-20211215200742151](ChapterFour.assets/image-20211215200742151.png)
...
![image-20211215201426879](ChapterFour.assets/image-20211215201426879.png)
...
AvlTree Insert(ElementType X,AvlTree T)
{
	...
	else
	{
		...
		{
			T->Left=Insert(X,T->Left);
			if(Height(T->Left)-Height(T->Right)==2)//不平衡
			{
				if(X<T->Left->Element)//左左，单旋转
				{
